...
def _header_band() -> Table:
    ...
    """
    band = Table(
        [["TNG RISE", "iTEKAD APPLICATION PACK"]],
...
def build_pdf(
    ...
    *,
    ...
    story.append(Paragraph(
        "<i>This pack was prepared automatically by TNG Rise using verified "
    ...
    story.append(Spacer(1, 8 * mm))
    story.append(Paragraph(
        "Generated by TNG Rise &middot; "
        f"Document ID: TR-{datetime.now().strftime('%Y%m%d-%H%M%S')} &middot; "
